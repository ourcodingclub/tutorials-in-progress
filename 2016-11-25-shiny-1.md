...
Notice that the name given to your app was assigned to the directory, not the app script file. For your app to work, the file must remain named `app.R`!

It is possible to create a Shiny app with two files called `ui.R` and `server.R', but the same can be accomplished by using one file. In the past, Shiny apps had to be created using two files, but the Shiny package has since been updated to allow the single file app structure, making it much easier to share and collaborate on one app. You will see some other tutorials on the internet using the old two file structure but these can be easily translated to the one file structure. This tutorial will assume you have the one file app structure.
...
Set up a similar directory structure for yourself using Windows Explorer, Finder, or some other file manager.
