...
- Get familiar with Shiny and its uses
...
It's possible to create a Shiny app with two files called `ui.R` and `server.R`, but the same can be accomplished by using one file. The difference between creating an app with two files and one file is... This tutorial will assume you have the one file app structure.

Local data for your app needs to be stored in a directory called `Data` inside your app directory. To create the directory `Data`, navigate to the `Files` tab in RStudio... When importing the data for use by `app.R` use the full file path, e.g.:
...
Looking at the app, we can see that the app has a `sidebarLayout` with a `sidebarPanel`, `mainPanel` and `titlePanel`. It uses `selectInput` to choose the genotype of barley shown in the histogram, and the colour of the histogram, a `sliderInput` to choose the number of bins in the histogram, and a `textInput` to display some text in the app. The histogram is located in the `mainPanel` along with a table of the data being shown, while the inputs are in the `sidebarPanel`.
...
- `inputId = "gen"` gives this input the name `gen` which will become useful when referencing this input later in the app script.
...
This creates a block of text that is coloured red (`style="color:red"`), within that block there is a paragraph of text (`tags$p("Visit us at:")`) and a link (`tags$a(href = "https://ourcodingclub.github.io", "Coding Club")`).
